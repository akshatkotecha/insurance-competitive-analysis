...
Prerequisites:
    pip install plotly pyodbc pandas requests
    set GROQ_API_KEY=your-groq-api-key    (optional — routing falls back to
                                            "unclear" without it. Free, no
                                            card, from console.groq.com)
"""
...
def get_conn():
    """A pyodbc connection is not safe to share across threads. Dash's dev
    server handles requests concurrently, so callbacks that fire together
    (e.g. every chart on first page load) can each land on a different
    worker thread — a single cached connection caused intermittent 500s
    under that load. One connection per thread, reused within it, fixes
    it without giving up reuse."""
...
# =====================================
# ANSWERING — front-end agnostic. Returns plain data (a Streamlit chat
# message and a Dash chat bubble render the same dict differently); the
# routing-to-answer logic itself lives here exactly once.
# =====================================

def answer_question(question):
    """Route a question and build its answer. Returns a dict:
        answer   -- markdown-ish text (uses ** for bold)
        table    -- a DataFrame to display, or None
        fig      -- a plotly figure to display, or None
        sql      -- the query behind the answer, or None
        kind     -- the routed intent
        source   -- "rules" or "model"
        answered -- False when nothing useful was found
    """
    slabs = load_slabs()
    slab_text = ", ".join(
    ...
    return {
        "answer": answer, "table": table, "fig": fig, "sql": sql_shown,
        "kind": kind, "source": plan.get("source"), "answered": answered,
    }
